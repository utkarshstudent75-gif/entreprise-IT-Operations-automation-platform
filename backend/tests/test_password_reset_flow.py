...
def test_password_reset_flow_works_end_to_end(sqlite_db, monkeypatch, caplog):
    user = User(
        username="ajay", email="ajaykumar@example.com", hashed_password="oldhash"
    )
    sqlite_db.add(user)
    sqlite_db.commit()
    ...
    reset_request = (
        sqlite_db.query(PasswordResetRequest).filter_by(user_id=user.id).one()
    )
    ...
    assert (
        password_reset_service.reset_password(
            sqlite_db, user.email, otp_code, "NewPassword@123"
        )
        is True
    )
    ...
    with pytest.raises(PasswordResetAlreadyUsedError):
        password_reset_service.reset_password(
            sqlite_db, user.email, otp_code, "AnotherPass@123"
        )


def test_expired_otp_is_rejected(sqlite_db):
    user = User(
        username="ajay2", email="expired@example.com", hashed_password="oldhash2"
    )
    sqlite_db.add(user)
    sqlite_db.commit()
    sqlite_db.refresh(user)
    ...
    with pytest.raises(PasswordResetExpiredError):
        password_reset_service.reset_password(
            sqlite_db, user.email, "000999", "NewPassword@123"
        )


def test_unknown_email_request_returns_success(sqlite_db):
    assert (
        password_reset_service.request_password_reset(sqlite_db, "missing@example.com")
        is True
    )
...
def test_verify_otp_rejects_invalid_code(sqlite_db):
    user = User(
        username="ajay3", email="mismatch@example.com", hashed_password="oldhash3"
    )
    sqlite_db.add(user)
    sqlite_db.commit()
    sqlite_db.refresh(user)
