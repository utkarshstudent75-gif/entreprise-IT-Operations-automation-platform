...
def test_invalid_token_rejected(client):
    response = client.get(
        "/api/v1/tickets", headers={"Authorization": "Bearer invalid_jwt_token_value"}
    )
    assert response.status_code == 401
    assert response.json()["error"]["code"] == "UNAUTHORIZED"
...
def test_role_based_access_control(client, db):
    ...
    response = client.get(
        "/api/v1/admin", headers={"Authorization": f"Bearer {helpdesk_token}"}
    )
    ...
    response = client.get(
        "/api/v1/admin", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert response.status_code == 200
