...
def restore_audit_session_local(monkeypatch):
    """
    Restore the real database SessionLocal in audit_service for E2E tests,
    overriding the SQLite mocking in conftest.py.
    """
    monkeypatch.setattr(audit_service_mod, "SessionLocal", RealSessionLocal)
...
def clean_postgres():
    ...
    try:
        ...
        db.execute(text("TRUNCATE TABLE audit_logs, password_reset_requests, users CASCADE"))
        db.commit()
...
def test_complete_e2e_workflow_and_validations(caplog):
    ...
    try:
        ...
        res = client.post("/api/v1/password/forgot-password", json={"email": "unknown@example.com"})
        assert res.status_code == 200
        ...
        reset_req = db.query(PasswordResetRequest).filter_by(user_id=user_id).order_by(PasswordResetRequest.created_at.desc()).first()
        ...
        # Verify valid OTP
        res = client.post("/api/v1/password/verify-otp", json={"email": email, "otp": otp_code})
        assert res.status_code == 200
        assert res.json()["success"] is True
        ...
        res = client.post("/api/v1/password/reset-password", json={
            "email": email,
            "otp": otp_code,
        ...
        res = client.post("/api/v1/password/forgot-password", json={"email": email})
        assert res.status_code == 200
        expired_req = db.query(PasswordResetRequest).filter_by(user_id=user_id, is_used=False).order_by(PasswordResetRequest.created_at.desc()).first()
        assert expired_req is not None
        expired_otp = expired_req.otp

        # Manually expire it in the database
        expired_req.expires_at = datetime.utcnow() - timedelta(minutes=1)
        db.commit()

        # Verify expired OTP is rejected on verify-otp
        res = client.post("/api/v1/password/verify-otp", json={"email": email, "otp": expired_otp})
        ...
        failed_audit_logs = db.query(AuditLog).filter(AuditLog.user_id == user_id, AuditLog.status == "FAILED").all()
        assert len(failed_audit_logs) > 0
        failed_reasons = [log.details.get("reason") for log in failed_audit_logs if log.details]
        ...
        request_id_val = res.headers["x-request-id"]

        # 14. Exception handling validation
        # Trigger validation exception (invalid email format)
        res = client.post("/api/v1/password/forgot-password", json={"email": "not-an-email"})
        ...
        # We already made 3 requests for email (2 valid, 1 expired).
