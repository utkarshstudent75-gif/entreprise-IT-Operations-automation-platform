...
def test_create_user_returns_user_response(monkeypatch):
    request = UserCreate(
        username="utkarsh", email="utkarsh@example.com", password="Password@123"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda db, username, email, hashed_password: DummyUser(
...
def test_create_user_conflict_on_existing_username(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.repositories.user_repository.user_repository.get_by_username",
        lambda db, username: DummyUser(
            id=1, username=username, email="existing@example.com"
        ),
    )
    monkeypatch.setattr(
        "app.repositories.user_repository.user_repository.get_by_email",
...
def test_create_user_conflict_on_existing_email(monkeypatch):
    request = UserCreate(
        username="newuser", email="utkarsh@example.com", password="Password@123"
    )
